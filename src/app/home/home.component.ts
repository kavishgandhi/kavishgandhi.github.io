...
export class HomeComponent implements OnInit {
  ...
  expansions_experience:expansionPanels[] = [
    ...
    summary: "The next exciting project that I worked on was that of developing a pilot to detect client specific retail products from the e-commerce websites' banners. For this task we tried and were not successful in using the off-the-shelf classification models, since the products were kinda specific to the client. Hence, we researched about different models availabble to use, and decided to implement MILDNet, which looked promising. And voila! We had developed a model which could match the products from the banners to the catalog with top-3 accuracy of 72%"},
    {title: 'Naive user can now train their own ML Models', keywords: 'ML-FastAPI-Automation', 
    summary: 'Since, we were the R&D team, many client facing teams would come to us with a request to build a custom ML model for prediction. And to do this everytime a dataset changed would be a wast of development time. So,we decided to build a service using which a naive user can train their own ML model (without any ML background) with their own custom dataset. This was proven to be successful since, it saved ~1200 hours of manaul work per month'},
    {title: 'Automating manual tasks saves time', keywords: 'Python-Automation', 
    summary: 'One of the always ongoing task(s) that I was assigned with was automating the manual work done by other team members using Python. This essentially helped save a lot of work for various client delivery teams'}
  ];
  expansions_projects:expansionPanels[] = [
    {title: 'InfoChain', keywords: 'BlockChain', 
    summary: 'Imagine a decentralized and distributed system of data storage and text services, no single entity having the entire responsibility of handling your personal data or the messages that you want to send. Such was our idea which we implemented using Blockchain as an underlying technology. Since it was completely new to us, we researched about available method using such decentralized methods and published a survey paper showing some of the best of the time. Then we went on to implement our own service using Ethereum Swarm(data) and Whisper(messages) and also published an implementation paper'},
    {title: 'GodsEye', keywords: 'ML-AI', 
    summary: 'Implemented as a challenge for TechGig hackthon (for which we were able to achieve 1st runner up position). Given a stream of video, we had to detect the total slots in a parking lot and determine using ML models whether it was empty or not, the coordinates, color and orientation of the car (in filled up slots)'},
    {title: 'Hastakshara', keywords: 'ML-AI', 
    summary: "Detect whether a given signature is forged or true, extremely useful for verifying useful documents. For this project we decide to implement Capsule networks, newly released at that time with a lot of potential. Yes, we could have used CNNs but given the disadvantages like max-pooling and spatial feature reduction, it will become easier to forge anyone's signature. We researched about and implemented the CapsNet model using tensorflow."},
    {title: 'SmartHome', keywords: 'IoT', 
    summary: 'Ever dreamnt of controlling things with your mobile or with your app. Well, that was our idea while working on SmartHome, an IoT based system which would enable people to control the lights, fans, basically anything which can be controlled with switches. And the most amazing part was that the current electrical components need not be changed for installing our system'}
  ];
